from sqlalchemy import Column, Integer, String, ForeignKey, Table, Text, create_engine, JSON
...
class School(Base, TimestampMixin):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True)  # School code from the website
    ...
    _services: Mapped[Optional[str]] = mapped_column('services', Text)  # Store as JSON string
...
class ImpartedStudy(Base, TimestampMixin):
    ...
    # Study details
    degree: Mapped[Optional[str]] = mapped_column(String)  # e.g., "Ciclos Formativos de FP de Grado Medio"
    family: Mapped[Optional[str]] = mapped_column(String)  # e.g., "SANIDAD"
    name: Mapped[str] = mapped_column(String, nullable=False)  # e.g., "Cuidados Auxiliares de Enfermería"
    modality: Mapped[Optional[str]] = mapped_column(String)  # e.g., "Diurno", "Vespertino", "Virtual"

    # Many-to-many relationship with School
    schools: Mapped[List["School"]] = relationship(
        "School",
        secondary=school_studies,
        back_populates="imparted_studies"
    )
